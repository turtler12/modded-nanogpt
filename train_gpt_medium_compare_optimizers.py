...
# medhaven: modified to compile flex_attention
# use of FlexAttention contributed by @KoszarskyB

from torch.nn.attention.flex_attention import (
    flex_attention as flex_attention_raw,
    create_block_mask,
    BlockMask,
)

# Compile flex_attention into its fused kernel so it doesn't materialize the full scores matrix.
compiled_flex_attention = torch.compile(flex_attention_raw)

def flex_attention(*args, **kwargs):
    return compiled_flex_attention(*args, **kwargs)

# Optional: keep inductor tuning flags if you really want them, but with
# TORCHDYNAMO_DISABLE=1 they won’t matter much. To be safe, you can comment out:
...
class GPT(nn.Module):
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        x = norm(x)
        # medhaven: modified to reduce peak memory usage during training
        # if self.training:
        ...
        #     return loss
        if self.training:
            # Select only the seed positions, as before
            x = x[seed_positions]            # [N, D]
            y = target_seq[seed_positions]   # [N]

            # Same normalization and projection as before
            x = F.layer_norm(x, [x.size(-1)], dtype=torch.bfloat16)
            logits = F.linear(x.flatten(end_dim=1), self.lm_head_w.bfloat16())
            logits = logits.float()
            logits = torch.nan_to_num(logits, 0.0, 0.0, 0.0)

            # ---- NEW: compute cross-entropy in chunks to reduce peak memory ----
            # We keep the *same* nonlinearity 15 * logits / sqrt(logits^2 + 225),
            # but we only materialize it on smaller slices.
            num_tokens = logits.shape[0]
            num_chunks = 4 if num_tokens >= 4 else 1  # you can increase to 8 if still OOM

            logits_chunks = torch.chunk(logits, num_chunks, dim=0)
            y_chunks = torch.chunk(y.view(-1), num_chunks, dim=0)

            total_loss = logits.new_zeros(())
            total_count = 0

            for lc, yc in zip(logits_chunks, y_chunks):
                # Apply the same squashing as before, but on a chunk
                lc = 15 * lc * torch.rsqrt(lc.square() + 225)

                # Compute summed loss for this chunk to keep gradients correct
                chunk_loss = F.cross_entropy(lc, yc, reduction="sum")
                total_loss = total_loss + chunk_loss
                total_count += yc.numel()

            # Global mean over all tokens = same as original reduction="mean"
...
# medhaven: commented out to use comparing optimizers code
...
# medhaven: commented out to used comparing optimizers code
...
# medhaven: code added to compare optimizers
# choose which optimizer recipe to use: "muon" (default), "adam", or "sgd"
optim_kind = os.environ.get("OPTIM_KIND", "muon").lower()

if optim_kind == "muon":
    # ----------------- original Muon+Adam split (baseline) -----------------
    # collect the parameters to optimize
    ...
    # init the optimizer(s) – exactly as in the original code
    adam_param_groups = [
...
elif optim_kind == "adam":
    # ----------------- pure AdamW on all parameters -----------------
    optimizer1 = torch.optim.AdamW(
        model.parameters(),
        lr=0.008,               # starting guess; you can tune this
        betas=(0.8, 0.95),
        eps=1e-10,
        weight_decay=0.0,
        fused=True,
    ...
    optimizer2 = None
    optimizers: list[torch.optim.Optimizer] = [optimizer1]

elif optim_kind == "sgd":
    # ----------------- pure SGD+momentum on all parameters -----------------
    optimizer1 = torch.optim.SGD(
        model.parameters(),
        lr=0.04,                # starting guess; you can tune this
        momentum=0.9,
        nesterov=True,
        weight_decay=0.0,
    )
    ...
    raise ValueError(f"Unknown OPTIM_KIND: {optim_kind}")
...
# medhaven: commented out bc not supported
...
for step in range(train_steps + 1):
    ...
    # medhaven: code replaced to compare optimizers
    # for group in optimizer2.param_groups:
    #     frac = min(step / 300, 1) # momentum warmup for muon
    #     group["momentum"] = (1 - frac) * 0.85 + frac * 0.95
    if optimizer2 is not None:
        # momentum warmup only for Muon
        for group in optimizer2.param_groups:
            frac = min(step / 300, 1)  # momentum warmup for muon
            group["momentum"] = (1 - frac) * 0.85 + frac * 0.95
