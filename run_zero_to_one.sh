...
#SBATCH --mem=80G
#SBATCH --output=/data/scratch/medhaven/logs/zero_to_1.log
#SBATCH --error=/data/scratch/medhaven/logs/zero_to_1.err
...
export MAP_RANGE="0_1"   # 0 → 1
export MAP_NORMALIZE=1

echo "[$(date)] RANGE=${MAP_RANGE} EPS=${MAP_EPS} (task ${SLURM_ARRAY_TASK_ID}) on ${HOSTNAME}"

# run (assumes cwd has train_gpt_mappings_with_svd.py, same as your other scripts)
